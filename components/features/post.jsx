...
export default function Post(props) {
    ...
    return (
        <div className={`post overlay-zoom overlay-dark ${post.type === 'video' ? 'post-video' : ''} ${adClass}`}>
            {
            ...
            <div className="post-details">
                <div className="post-meta">
                    <ALink href="#" className="post-author">{post.author}</ALink> on <ALink href="#" className="post-date">{new Date(post.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: "2-digit", timeZone: "UTC" })}</ALink> | <ALink href="#" className="post-comment"><span>{post.comments}</span> Comments</ALink>
                </div>
                ...
                {
                    isButton ?
                        <ALink href={`/blog/single/${post.slug}`} className={`btn btn-link btn-underline btn-sm text-capitalize font-weight-normal ${btnAdClass}`}>{btnText}<i className="d-icon-arrow-right"></i></ALink>
                        : ''
                }
            </div>
        </div >
